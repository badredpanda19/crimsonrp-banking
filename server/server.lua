...
VORPcore.Callback.Register('vorp_bank:getinfo', function(source, cb, bankName)
    ...
    -- Search by character identifier to only get accounts for THIS character
    print("^3[vorp_banking] DEBUG: Querying bank_users for charidentifier:^0", charidentifier)
    MySQL.query("SELECT * FROM bank_users WHERE charidentifier = @charidentifier",
        { charidentifier = charidentifier }, function(allRows)
            if allRows then
                print("^3[vorp_banking] DEBUG: Found^0", #allRows, "^3rows in bank_users^0")
                for i, row in ipairs(allRows) do
                    print("^3[vorp_banking] DEBUG: Row^0", i, "^3name:^0", row.name, "^3unpaid_salary:^0", row.unpaid_salary)
                end
            else
                print("^3[vorp_banking] DEBUG: No rows found in bank_users for this character^0")
            end
            local salariesMap = {}
            local totalSalary = 0
            local filteredBanksMap = {}
            local currentBankInfo = nil

            if allRows then
                for _, row in ipairs(allRows) do
                    -- Count all unpaid salaries across all rows linked to this character
                    local unpaidSalary = tonumber(row.unpaid_salary) or 0
                    if unpaidSalary > 0 then
                        totalSalary = totalSalary + unpaidSalary
                        
                        local sLabel = ""
                        if string.sub(row.name or "", 1, 8) == "Salary: " then
                            sLabel = string.sub(row.name, 9)
                        else
                            -- If it's a normal bank row with salary, add it to general
                            sLabel = "society paycheck (".. (row.name or "Unknown") ..")"
                        end
                        salariesMap[sLabel] = (salariesMap[sLabel] or 0) + unpaidSalary
                    end

                    -- Separate bank accounts from salary storage rows
                    if string.sub(row.name or "", 1, 8) ~= "Salary: " then
                        local bName = row.name
                        if not bName or bName == "" then bName = bankName end -- Fallback to current bank name if row has no name
                        
                        if not filteredBanksMap[bName] then
                            row.name = bName
                            row.salary = row.unpaid_salary
                            filteredBanksMap[bName] = row
                        else
                            -- Aggregate money/gold if duplicates exist in DB
                            filteredBanksMap[bName].money = filteredBanksMap[bName].money + row.money
                            filteredBanksMap[bName].gold = filteredBanksMap[bName].gold + row.gold
                        end
                    end
                end
            end

            -- Ensure the current bank exists in the map
            currentBankInfo = filteredBanksMap[bankName]
            ...
            if not currentBankInfo then
                ...
                filteredBanksMap[bankName] = currentBankInfo
            end

            -- Convert maps back to lists for UI
            local salaries = {}
            for name, amount in pairs(salariesMap) do
                table.insert(salaries, { name = name, amount = amount })
            end

            local filteredBanks = {}
            for _, b in pairs(filteredBanksMap) do
                table.insert(filteredBanks, b)
            end
